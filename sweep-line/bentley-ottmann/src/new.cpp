/*
	...
	* No hay intersecciones en los extremos de los segmentos (salvo con segmentos verticales).

	* Actualización: Parece funcionar para líneas verticales ahora. No se ha probado exhaustivamente.
*/
...
struct event { //evento en la cola
	double x;
	int tp, id1, id2; //type {0: inter, 2:open, -1:end, 1:vertical, -3: null} , seg_ids
	// i need open to be performed before vertical, but vertical to be performed before inter and end
	event(){}
...
void input(bool isrand = false){ 
	...
	if (isrand){ 
		...
		for(int i = 0; i<5; i++){
			double nx = (mxc * i) / 5.0;
			double ny = genrand_real(0.00, mxc);
			seg aux;
			aux.p.x = aux.q.x = nx + 1.00;
			aux.p.y = 0.00;
			aux.q.y = mxc;
			aux.id = n+i;
			lines.push_back(aux);
...
void initialize(){
	...
	for (int i = 0; i < lines.size(); i++){
		seg &t = lines[i];
		if ((fabs(t.p.x - t.q.x) <= EPS)) pq.insert(event(t.left(), 1, i));
		else {
			pq.insert(event(t.left(), 2, i));
...
vector<pt> intersection_points(){
	...
	while(pq.size()){
		...
		if (e.tp == 2){ //add seg
		...
		} else if (e.tp == 1) { //vertical line, the status order doesnt change so just handle separately
			if (last.tp == -3 or s.empty()) continue; // need to have processed atleast 1 event before
			seg cur = lines[e.id1];
			seg aux;
			aux.p = { last.x, min(cur.p.y, cur.q.y) };
			aux.q = { cur.p.x+EPS, aux.p.y };
			auto it = s.lower_bound(aux);
			auto itl = previous(it);
			while (itl != s.end() && inter(*itl, cur, auxl, auxr)) out.push_back(auxl), it = previous(it);
			while (it != s.end() && inter(*it, cur, auxl, auxr)) out.push_back(auxl), it++;
			...
			assertm(cur2<cur1, "PRECISION ERROR"); //make sure cur2 is now below cur1
...
int intersection_points_count(){
	...
	while(pq.size()){
		...
		if (e.tp == 2){
			...
			auto it = s.lower_bound(aux);
			auto itl = previous(it);
			while (itl != s.end() && inter(*itl, cur, auxl, auxr)) out++, it = previous(it);
			while (it != s.end() && inter(*it, cur, auxl, auxr)) out++, it++;
		} else { //intersection
			auto it1 = where[e.id1], it2 = where[e.id2]; // cur1 below cur2, have to swap them
			seg cur1 = *it1, cur2 = *it2, copy_cur1 = cur1, copy_cur2 = cur2;
